...
QImage* Scene::getSceneImage(){
    ...
    //Set the Border
    LineObject b0(0,0,1.0,0,qRgba(0,0,0,255),0.0,0,0);
    for(int i = 0; i < LineObject::getSize(); i++)
        SceneImage->setPixel(0,i,b0.getAttributeAsPixel(i));

    LineObject b1(1.0,0,1.0,1.0,qRgba(0,0,0,255),0.0,0,0);
    for(int i = 0; i < LineObject::getSize(); i++)
        SceneImage->setPixel(1,i,b1.getAttributeAsPixel(i));

    LineObject b2(1.0,1.0,0,1.0,qRgba(0,0,0,255),0.0,0,0);
    for(int i = 0; i < LineObject::getSize(); i++)
        SceneImage->setPixel(2,i,b2.getAttributeAsPixel(i));

    LineObject b3(0,1.0,0,0,qRgba(0,0,0,255),0.0,0,0);
    for(int i = 0; i < LineObject::getSize(); i++)
